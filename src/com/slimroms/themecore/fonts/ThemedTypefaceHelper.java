...
public class ThemedTypefaceHelper {
    ...
    public void load(Context context, Overlay overlay) {
        try {
            loadThemedFonts(context, overlay);
            return;
        } catch(Exception e) {
            Log.w(TAG, "Unable to parse and load themed fonts for " + overlay +
    ...
    private void loadThemedFonts(Context context, Overlay overlay) throws Exception {
        //Parse the font XML
        File fontsXml = new File(overlay.tag, "fonts.xml");
        FontListParser.Config fontConfig = FontListParser.parse(new File(overlay.tag, "fonts.xml"),
                overlay.tag);
